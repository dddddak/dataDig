...
SELECT Name, ROUND(AVG(Total+HP+Attack+Defense)) as avg_total, MAX(HP, Attack, Defense, Speed) as max_value, MIN(HP, Attack, Defense, Speed) as min_value FROM pokemon GROUP BY Name;


-- those numbers per category in the data (using HAVING)
SELECT Type_1, COUNT(*) as n_type_1 FROM pokemon GROUP BY Type_1 HAVING COUNT(*) >= 1;

SELECT Type_2, COUNT(*) as n_type_2
FROM pokemon GROUP BY Type_2 HAVING COUNT(*) >= 1;

--  ways are there to group the data values that don’t exist yet (using CASE)
SELECT Name, Type_1, Type_2, 
    CASE
        WHEN Type_2 IS NULL THEN "NOT DEFINED"
        ELSE " "
    END as "n/a"
FROM pokemon ORDER BY Name;

-- interesting ways are there to filter the data (using AND/OR)
SELECT Type_1, Type_2, Name FROM pokemon WHERE Type_1 = "Fire" AND Type_2 = "Flying" ;

SELECT Type_1, Type_2, Name FROM pokemon WHERE Type_1 = "Water" OR Type_2 = "Poison" ;
